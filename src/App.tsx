...
import { generateQuest, generateTopicExplanation, animateTopicImage, getChatResponse, fetchWikipediaExtract } from './services/groqService';
import { Quest, QuestType } from './services/groqService';
...
const LiveAudioSession = ({ onClose }: { onClose: () => void }) => {
  ...
  const startSession = async () => {
    ...
    setTimeout(async () => {
      ...
      setIsActive(true);
      setTranscript("Listening... Say a topic like 'Jupiter' and I will tell you about it.");
      
      // Simulate user speaking after 3 seconds
      setTimeout(async () => {
        setTranscript("User: Tell me about the Roman Empire");
        
        // Fetch response from our existing Wikipedia service!
        const response = await getChatResponse("Roman Empire", []);
        setTranscript(`EduBot: ${response.replace(/\[.*\]\(.*\)/g, '')}`); // Strip markdown links for speech
...
const Dashboard = ({ onClose }: { onClose: () => void }) => {
  ...
  return (
    <motion.div 
      ...
      className="fixed inset-0 z-[150] bg-black/90 backdrop-blur-xl flex items-center justify-center p-6"
    >
      <div className="max-w-4xl w-full glass rounded-[3rem] p-12 space-y-8 relative max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center">
          <div className="space-y-1">
            <h2 className="text-3xl font-bold flex items-center gap-3">
              <Users className="text-primary" size={32} /> User Dashboard
            </h2>
            <p className="text-slate-400">Manage students and track their progress</p>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-white/10 rounded-xl transition-colors">
            <XCircle size={32} className="text-slate-500 hover:text-white" />
          </button>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          <div className="md:col-span-1 space-y-6">
            <div className="p-6 rounded-2xl bg-white/5 border border-white/10 space-y-4">
              <h3 className="font-bold text-lg flex items-center gap-2"><Database size={20} className="text-emerald-400"/> Add User</h3>
              {error && <div className="p-3 bg-red-500/20 border border-red-500/50 text-red-100 rounded-xl text-sm">{error}</div>}
              <form onSubmit={handleSubmit} className="space-y-4">
                <input 
                  ...
                  className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-sm outline-none focus:border-primary"
                  required
                />
                <input 
                  type="email" 
                ...
                >
                  {isLoading ? <Loader2 className="animate-spin" size={18} /> : <span>Save User</span>}
                </button>
              </form>
            </div>
          </div>
          ...
          <div className="md:col-span-2">
            <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
              <table className="w-full text-sm text-left">
                <thead className="text-xs text-slate-400 uppercase bg-black/30">
                  <tr>
                ...
                <tbody className="divide-y divide-white/5">
                  {users.length === 0 ? (
                    <tr><td colSpan={4} className="px-6 py-8 text-center text-slate-500">No users found. Add one to get started.</td></tr>
                  ) : (
                    users.map(u => (
                      <tr key={u.id} className="hover:bg-white/5 transition-colors">
                        <td className="px-6 py-4 font-bold">{u.name}</td>
                        <td className="px-6 py-4 text-slate-400">{u.email}</td>
                        <td className="px-6 py-4 text-emerald-400 font-mono">{u.points}</td>
...
const LoginPage = ({ onLogin }: { onLogin: (user: any) => void }) => {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col lg:flex-row bg-slate-950 overflow-hidden relative selection:bg-primary/30">
      {/* Decorative ambient background */}
      <div className="absolute inset-0 z-0 pointer-events-none">
        <div className="absolute top-[-10%] left-[-10%] w-[40vw] h-[40vw] bg-primary/20 blur-[120px] rounded-full mix-blend-screen" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[40vw] h-[40vw] bg-secondary/20 blur-[120px] rounded-full mix-blend-screen" />
      </div>

      {/* Left Content Area (Hero) */}
      <div className="flex-1 hidden lg:flex flex-col justify-center p-20 relative z-10 border-r border-white/5">
        <motion.div 
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 text-primary text-sm font-bold tracking-wide backdrop-blur-md">
            <Sparkles size={16} className="animate-pulse" />
            <span>Next-Generation Learning</span>
          </div>
          
          <h1 className="text-6xl xl:text-7xl font-black leading-[1.1] tracking-tight text-white">
            Unlock Your <br />
          ...
          <p className="text-lg xl:text-xl text-slate-400 leading-relaxed font-medium">
            Join thousands of learners mastering new skills with AI-powered interactive quests, real-time voice tutoring, and stunning video generation.
          </p>

          <div className="grid grid-cols-2 gap-6 pt-8">
            {[
              { icon: <BrainCircuit className="text-primary" size={24}/>, title: "AI Generation", desc: "Dynamic quests tailored for you" },
              { icon: <Trophy className="text-secondary" size={24}/>, title: "Gamified XP", desc: "Earn points as you learn" }
            ].map((feat, idx) => (
              <motion.div 
                key={idx}
                whileHover={{ y: -5 }}
                className="p-5 rounded-3xl bg-white/5 border border-white/10 backdrop-blur-sm shadow-xl shadow-black/20"
              >
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-20 relative z-10">
        ...
        >
          <div className="glass rounded-[2.5rem] p-10 lg:p-12 shadow-2xl shadow-black/50 border border-white/10 relative overflow-hidden backdrop-blur-2xl bg-slate-900/60">
            {/* Top glowing accent */}
            <div className="absolute top-0 inset-x-0 h-1 bg-gradient-to-r from-transparent via-primary to-transparent opacity-50" />

            <div className="text-center space-y-6 mb-10">
              <div className="w-20 h-20 bg-gradient-to-br from-primary to-secondary p-[2px] mx-auto rounded-3xl shadow-[0_0_40px_rgba(99,102,241,0.3)]">
              ...
              <div>
                ...
                <p className="text-slate-400 text-sm">
                  {isLoginMode ? "Enter your credentials to continue your journey." : "Create a new student profile to get started."}
                </p>
              </div>
            </div>
            ...
            {error && (
              <motion.div 
                ...
                className="mb-8 p-4 bg-red-500/10 border border-red-500/20 text-red-400 rounded-2xl text-sm flex items-center gap-3 overflow-hidden"
              >
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="space-y-1.5">
                ...
                <input 
                  ...
                  value={name}
                  onChange={e => setName(e.target.value)}
                  className="w-full bg-black/40 border border-white/10 rounded-2xl px-5 py-4 text-white placeholder:text-slate-600 outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all shadow-inner"
                  required
                />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="pt-2">
                ...
                >
                  ...
                  <div className="relative px-6 py-4 flex items-center justify-center gap-2 font-bold text-white tracking-wide">
                    {isLoading ? <Loader2 className="animate-spin" size={20} /> : (isLoginMode ? "Start Exploring" : "Create Profile")}
            ...
            <div className="mt-8 text-center">
              ...
              >
                {isLoginMode ? (
                  <>New student? <span className="text-primary font-bold hover:underline">Create an account</span></>
                ) : (
                  <>Already have an account? <span className="text-primary font-bold hover:underline">Log in here</span></>
                )}
              </button>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="hidden lg:flex items-center gap-2">
              ...
              <button 
                ...
                className="p-2 hover:bg-white/5 rounded-xl text-slate-400 hover:text-emerald-400 transition-colors flex items-center gap-2"
              >
                <Database size={18} /> <span className="text-xs font-bold">Dashboard</span>
              </button>
            </div>
            <button 
            ...
            <div className="flex items-center gap-3 border-l border-white/10 pl-3">
              <div className="text-right hidden sm:block">
                ...
                <div className="text-xs text-emerald-400 font-mono">{points} XP</div>
              </div>
              <button 
                onClick={() => setCurrentUser(null)}
                className="px-3 py-1.5 bg-white/5 text-slate-300 rounded-lg font-bold text-xs hover:bg-red-500/20 hover:text-red-400 transition-all border border-white/10 hover:border-red-500/30"
              >
                Log Out
              </button>
            </div>
